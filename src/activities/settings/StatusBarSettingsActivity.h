...
class StatusBarSettingsActivity final : public UiListActivity {
 public:
  ...
  static constexpr int MAX_STATUS_BAR_ITEMS = 7;
 ...
 private:
  ...
  int listCount() const override { return MAX_STATUS_BAR_ITEMS; }
  ...
  // Labels are set once in onEnter(); buildScreen() only refreshes the
  // live value text (rowValues_) by
